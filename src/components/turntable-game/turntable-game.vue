<template>
  <transition name="turntable-game-slide">
    <div ref="gameWrapper" class="turntable-game" v-show="isVisible" :style="{backgroundImage: `url(${gameInfo.lotteryBgUrl})`}">
...
<script>
  ...
  import {fetchEnabledGame, fetchGameInfo, fetchValidLotteryCount, playGame} from '../../api/prize'
  ...
  const COMPONENT_NAME = 'turntable-game'
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        ...
        this.boxWidth = this.$refs.gameWrapper.offsetWidth - 40
        this.colorList = ['#FE2E8B', '#FE58A3']
        this.rotateCount = 0
      ...
      startRotate() {
        ...
        * */
        fetchValidLotteryCount(this.gameInfo.id).then(res => {
          if (res.end) {
            this._closeToast()
            this._openAlert('biubiubiu～您来晚了一步，礼品已发完了哟，但您和购物广场的故事才刚开始，超多惊喜等着您！', {
              onConfirm: () => {
                this.isVisible = false
              }
            })
      ...
      getGameInfo() {
        ...
        fetchEnabledGame().then(res => {
          ...
          if (this.gameInfo.id) {
            ...
            this._closeToast()
            this._openAlert('抽奖信息获取失败,暂时无法抽奖', {
              onConfirm: () => {
                this.isVisible = false
              }
            })
...
<style lang="stylus" scoped rel="stylesheet/stylus">
  .turntable-game
    ...
    &.turntable-game-slide-enter-active,&.turntable-game-slide-leave-active
      transition:all .3s
    &.turntable-game-slide-enter,&.turntable-game-slide-leave-to
    ...
    .game-box
      ...
      .box-wrapper
        ...
        .radius-bg,.radius-group
          ...
          background-size:100% 100%
        .radius-group
          background-color: #fe2e8b
          border-radius:50%
        .radius-bg
          &.default
            background-image:url(./radius-1.png)
          &.toggle
            background-image:url(./radius-2.png)
